...
public class Heapfile implements Filetype,  GlobalConst {
  ...
  private     String 	 _fileName;
	private HashMap<Integer, Integer> pageOffset;
	private HashMap<Integer, Integer> dirPageOffset;
	private HashMap<Integer, Integer> offsetToPage;
	private List<Integer> deletedDirOffsets;
	private Integer numRecordsPerDataPage;
	private Integer numRecordsPerDirPage;
	private Integer recordSize;
	private boolean columnarHeapFile;
  ...
  */
  protected HFPage _newDatapage(DataPageInfo dpinfop)
	...
	throw new HFException(null, "can't new pae");
      ...
      dpinfop.recct = 0;
      dpinfop.availspace = (short)hfpage.available_space();
  ...
  */
  private boolean  _findDataPage( RID rid,
	...
	public List<PageId> getListOfPagesInTheHeapFile()
			throws InvalidSlotNumberException, InvalidTupleSizeException, IOException, HFBufMgrException, HFException {
		...
		List<PageId> pageNoArrays = new ArrayList<PageId>();
		while (currentDirPageId.pid != INVALID_PAGE) {
			for (currentDataPageRid = currentDirPage
				...
				DataPageInfo dpinfo = new DataPageInfo(atuple);
				pageNoArrays.add(dpinfo.pageId);
			}
			nextDirPageId = currentDirPage.getNextPage();
			try {
				unpinPage(currentDirPageId, false /* undirty */);
	...
	public int findPosition(RID rid)
			throws Exception
	{
		if(pageOffset==null) {
			loadPositionBuffer();
		}
		if(pageOffset.containsKey(rid.pageNo.pid)) {
			return (numRecordsPerDataPage*pageOffset.get(rid.pageNo.pid))+rid.slotNo;
		} else {
			throw new Exception("Invalid RID");
		}
	}

	public RID findRID(int position)
			throws Exception
	{
		if(offsetToPage==null) {
			loadPositionBuffer();
		}
		int offset = position/numRecordsPerDataPage;
		if(offsetToPage.containsKey(offset)) {
			int pid = offsetToPage.get(offset);
			int slotNo = position - (offset*numRecordsPerDataPage);
			return new RID(new PageId(pid), slotNo);
		} else {
			throw new Exception("Invalid Position");
		}
	}

	public void printDirMetaInfo() throws Exception
	{
		PageId currentDirPageId = new PageId(_firstDirPageId.pid);
		...
		int dataPageCount = 0;
		int dirPageCount = 0;
		int noOfDPInfoPerDirPage = 0;
		boolean flag = true;
		ArrayList<Integer> dirPageList = new ArrayList<>();
		ArrayList<Integer> dataPageList = new ArrayList<>();

		while (currentDirPageId.pid != INVALID_PAGE) {
			dirPageList.add(currentDirPageId.pid);
			dirPageCount++;
			...
			for(currentDataPageRid = currentDirPage.firstRecord();
				...
				atuple = currentDirPage.getRecord(currentDataPageRid);
				DataPageInfo dpinfo = new DataPageInfo(atuple);
				dataPageList.add(dpinfo.pageId.pid);
				if(flag) {
					noOfDPInfoPerDirPage++;
				}
			}
			flag = false;
		...
		System.out.println("dirPageCount: "+dirPageCount);
		System.out.println("noOfDPInfoPerDirPage: "+noOfDPInfoPerDirPage);
		System.out.println("dataPageCount: "+dataPageCount);
		System.out.println("dirPages: "+dirPageList);
		System.out.println("dataPages: "+dataPageList);
	}

	public void loadPositionBuffer() throws Exception
	{
		PageId currentDirPageId = new PageId(_firstDirPageId.pid);
		...
		pinPage(currentDirPageId, currentDirPage, false/*read disk*/);
		if(currentDirPage == null) {
			invalidatePositionBuffer();
			throw new HFException(null, "pinPage return null page");
		}
		pageOffset = new HashMap<>();
		offsetToPage = new HashMap<>();
		dirPageOffset = new HashMap<>();

		Tuple atuple = new Tuple();
		int dirPageIndex = 0;

		while (currentDirPageId.pid != INVALID_PAGE) {
			dirPageOffset.put(currentDirPageId.pid, dirPageIndex);
			RID prevDataPageRid = null;
			int pageIndex = dirPageIndex * numRecordsPerDirPage;
			int prevSlotNo = 0;
			for(currentDataPageRid = currentDirPage.firstRecord();
				currentDataPageRid != null;
				currentDataPageRid = currentDirPage.nextRecord(currentDataPageRid)) {
				...
				if(prevDataPageRid==null) {
					pageIndex += currentDataPageRid.slotNo - prevSlotNo;
				} else {
					pageIndex += currentDataPageRid.slotNo - prevDataPageRid.slotNo;
				}
				DataPageInfo dpinfo = new DataPageInfo(atuple);
				pageOffset.put(dpinfo.pageId.pid, pageIndex);
				offsetToPage.put(pageIndex, dpinfo.pageId.pid);

				prevDataPageRid = currentDataPageRid;
			}
	...
	public void invalidatePositionBuffer() throws Exception
	{
		pageOffset = null;
		offsetToPage = null;
		dirPageOffset = null;
	...
	public void resetDeletedDirOffsetList() throws Exception
	{
		deletedDirOffsets = new ArrayList<>();
	}
  ...
  public  Heapfile(String name)
	...
	public  Heapfile(String name, int recordSize)
			throws HFException,
			HFBufMgrException,
			HFDiskMgrException,
			IOException

	{
		...
		columnarHeapFile = true;
		deletedDirOffsets = new ArrayList<>();
		numRecordsPerDataPage = (MINIBASE_PAGESIZE - HFPage.DPFIXED)/(HFPage.SIZE_OF_SLOT + recordSize);
		numRecordsPerDirPage = (MINIBASE_PAGESIZE - HFPage.DPFIXED)/(HFPage.SIZE_OF_SLOT + DataPageInfo.size);
	}
  ...
  public RID insertRecord(byte[] recPtr) 
		  ...
		  if (nextDirPageId.pid != INVALID_PAGE) 
			...
			throw new HFException(null, "can't new pae");
		      ...
		      currentDirPage = new HFPage(nextDirPage);
				...
				// To update directory page index info
				if(columnarHeapFile) {
					if (dirPageOffset != null) {
						dirPageOffset.put(currentDirPageId.pid, dirPageIndex+1);
					}
				}
	...
	throw new HFException(null, "can't find Data page");
      ...
      dpinfo.recct++;
      dpinfo.availspace = (short)currentDataPage.available_space();
      ...
      unpinPage(currentDirPageId, true /* = DIRTY */);
		...
		//Only for columnar heap files to update position buffer
		if(columnarHeapFile) {
			if(pageOffset!=null) {
				int pageIndex = dirPageIndex*numRecordsPerDirPage + currentDataPageRid.slotNo;
				if(pageOffset.containsKey(dpinfo.pageId.pid)) {
					if(pageOffset.get(dpinfo.pageId.pid)!=pageIndex) {
						throw new Exception("Calculated position value wrong");
					}
				} else {
					pageOffset.put(dpinfo.pageId.pid, pageIndex);
					offsetToPage.put(pageIndex, dpinfo.pageId.pid);
  ...
  public boolean deleteRecord(RID rid)  
      ...
      if (pdpinfo.recct >= 1) 
	{
	  ...
	  pdpinfo.availspace = (short)currentDataPage.available_space();
      ...
      else
	{
	  ...
	  if((currentDataPageRid == null)&&(pageId.pid != INVALID_PAGE))
	      ...
	      if(pageId.pid != INVALID_PAGE)
		{
		  ...
		  //Recording deleted dir offset for other activities
	      if(columnarHeapFile) {
			  deletedDirOffsets.add(dirPageOffset.get(currentDirPageId.pid));
		  }
  ...
  private void delete_file_entry(String filename)
	...
	public List<Integer> getDeletedDirOffsets() {
		return deletedDirOffsets;
	}

	public Integer getNumRecordsPerDataPage() {
		return numRecordsPerDataPage;
	}

	public Integer getNumRecordsPerDirPage() {
		return numRecordsPerDirPage;
	}
}// End of HeapFile
